...
class MultiTaskTrainer:
    def __init__(self, model, train_loader, test_loader, device='cuda', test_person=-1):
    ...
    def _setup_phase_EEGnet(self):
        """阶段1：特征提取参数优化"""
        self._freeze_all()
        # 解冻特征提取网络
        for param in self.model.eeg_net.parameters():
    ...
    def _setup_phase_EYEnet(self):
        """阶段1：特征提取参数优化"""
        self._freeze_all()
        # 解冻特征提取网络
        for param in self.model.eye_net.parameters():
            param.requires_grad = True
        # 创建阶段专属优化器和调度器
        self.phase1_optimizer = optim.AdamW(
    ...
    def _setup_phase_PPSnet(self):
        """阶段1：特征提取参数优化"""
        self._freeze_all()
        # 解冻特征提取网络
        for param in self.model.pps_net.parameters():
            param.requires_grad = True
        # 创建阶段专属优化器和调度器
        self.phase1_optimizer = optim.AdamW(
    ...
    def train_epoch_phase_eeg(self, epoch):  # 训练特征提取模块和对比损失
        self.model.train()
        optimizer = self._setup_phase_EEGnet()

        total = {'loss': 0, 'a_loss': 0, 'v_loss': 0, 'c_loss': 0, 'a_acc': 0, 'v_acc': 0}

        pbar = tqdm(self.train_loader, desc=f'Train Epoch {epoch} (Phase EEGnet)')
        for eeg, eye, pps, arousal, valence in pbar:
            ...
            optimizer.zero_grad()
            # 前向传播
            a_out, v_out, c_loss1, c_loss2, c_loss3 = self.model(*inputs, labels=labels)
            ...
            total['c_loss'] += c_loss1.item() * batch_size
    ...
    def train_epoch_phase_eye(self, epoch):  # 训练特征提取模块和对比损失
        self.model.train()
        optimizer = self._setup_phase_EYEnet()

        total = {'loss': 0, 'a_loss': 0, 'v_loss': 0, 'c_loss': 0, 'a_acc': 0, 'v_acc': 0}

        pbar = tqdm(self.train_loader, desc=f'Train Epoch {epoch} (Phase EYEnet)')
        for eeg, eye, pps, arousal, valence in pbar:
            inputs = (
                eeg.to(self.device).float(),
                eye.to(self.device).float(),
            ...
            total['a_loss'] += 0  # 不计算arousal loss
            total['v_loss'] += 0  # 不计算valence loss
            total['c_loss'] += c_loss2.item() * batch_size
            total['a_acc'] += metrics['a_acc'] * batch_size
            total['v_acc'] += metrics['v_acc'] * batch_size
    ...
    def train_epoch_phase_pps(self, epoch):  # 训练特征提取模块和对比损失
        self.model.train()
        optimizer = self._setup_phase_PPSnet()

        total = {'loss': 0, 'a_loss': 0, 'v_loss': 0, 'c_loss': 0, 'a_acc': 0, 'v_acc': 0}

        pbar = tqdm(self.train_loader, desc=f'Train Epoch {epoch} (Phase PPSnet)')
        for eeg, eye, pps, arousal, valence in pbar:
            inputs = (
                eeg.to(self.device).float(),
                eye.to(self.device).float(),
            ...
            total['a_loss'] += 0  # 不计算arousal loss
            total['v_loss'] += 0  # 不计算valence loss
            total['c_loss'] += c_loss3.item() * batch_size
            total['a_acc'] += metrics['a_acc'] * batch_size
            total['v_acc'] += metrics['v_acc'] * batch_size
    ...
    def evaluate(self, mode='test'):
        ...
        with torch.no_grad():
            for eeg, eye, pps, arousal, valence in loader:
                ...
                c_loss = c_loss1 + c_loss2 + c_loss3
                # 统计指标
                metrics = self._compute_metrics((a_out, v_out), labels)
                batch_size = labels[0].size(0)
                total['loss'] += total_loss.item() * batch_size
    ...
    def run(self, epochs_phaseEEG, epochs_phaseEYE, epochs_phasePPS, epochs_phase2, epochs_phase3):
        global model_name
        print("Phase EEGnet : Training Feature Extractors with Contrastive Loss")
        for epoch in range(1, epochs_phaseEEG + 1):
            # 训练阶段1
            train_metrics = self.train_epoch_phase_eeg(epoch)
        ...
        print("Phase EYEnet : Training Feature Extractors with Contrastive Loss")
        for epoch in range(1, epochs_phaseEYE + 1):
            # 训练阶段1
            train_metrics = self.train_epoch_phase_eye(epoch)
        ...
        print("Phase PPSnet : Training Feature Extractors with Contrastive Loss")
        for epoch in range(1, epochs_phasePPS + 1):
            # 训练阶段1
            train_metrics = self.train_epoch_phase_pps(epoch)
        ...
        for epoch in range(1, epochs_phase3 + 1):
            ...
            model_name = f"TestPerson{self.test_person}_ArousalAcc{test_metrics['a_acc']:.2f}_ValenceAcc{test_metrics['v_acc']:.2f}"
            # # 早停检查
            # if self.early_stopping(test_metrics['loss']):
            #     break
